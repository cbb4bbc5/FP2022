...
module TwoPlayerGame where
    ...
    class Monad m => TwoPlayerGame m where
        ...
        type MoveB m :: *
        type GameState m :: *
        moveA :: GameState m -> m (MoveA m)
        moveB :: GameState m -> m (MoveB m)

    data Score = AWins | Draw | BWins deriving (Eq,Show,Read)
    turn :: TwoPlayerGame m => (GameState m -> Maybe Score) -> (GameState m -> MoveA m -> GameState m) -> (GameState m -> MoveB m -> GameState m) -> GameState m -> m Score
    ...
    type Board4 = [[Score]]
    fourInARow :: (TwoPlayerGame m,MoveA m ~ Int,MoveB m ~ Int,GameState m ~ Board4) => m Score
    ...
    type Board9 = [[Score]]
    ticTacToe :: (TwoPlayerGame m,MoveA m ~ (Int,Int),MoveB m ~ (Int,Int),GameState m ~ Board9) => m Score
    ...
    instance (Show s, Read a, Read b) => TwoPlayerGame (IOGame s a b) where
        type MoveA (IOGame s a b) = a
        type MoveB (IOGame s a b) = b
        type GameState (IOGame s a b) = s
    ...
    instance TwoPlayerGame (GameTree s a b) where
        type MoveA (GameTree s a b) = a
        type MoveB (GameTree s a b) = b
        type GameState (GameTree s a b) = s
